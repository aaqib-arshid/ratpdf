#!/usr/bin/env python3
"""Generate GuideBodiesWave54.cs — CREATE batch 18 (17 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave54.cs"
...
def body(slug: str) -> str:
    ...
    if slug in EDITOR_CREATE_SLUGS:
        cluster = W54_CREATE_CLUSTER + W54_EDITOR_CLUSTER + W48_PERSONA_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPARE_MEGA + COMPARE_SMB + COMPARE_DEEP + SHARED_MEGA + COMPLIANCE_MEGA
        topup = W6_1500 + W54_COMPRESS_TOPUP
        closing = W54_CLOSING + W6_CLOSING
    elif slug in WORKFLOW_CREATE_SLUGS:
        cluster = W54_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W54_CLOSING + W6_CLOSING
    else:
        cluster = W54_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W54_COMPRESS_TOPUP
        closing = W54_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W54_SLUGS)} guides)")
