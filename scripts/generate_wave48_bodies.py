#!/usr/bin/env python3
"""Generate GuideBodiesWave48.cs — CREATE batch 12 (17 persona editorial guides)."""
...
from wave47_expansions import W47_PERSONA_CLUSTER
from wave48_expansions import (
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave48.cs"
...
def body(slug: str) -> str:
    ...
    topup = W6_1500 + W48_COMPRESS_TOPUP
    closing = W48_CLOSING + W6_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W48_SLUGS)} guides)")
