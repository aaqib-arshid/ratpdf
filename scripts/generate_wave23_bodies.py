#!/usr/bin/env python3
"""Generate GuideBodiesWave23.cs — Beginner basics + how-to + checklist + glossary batch 5."""
...
from wave16b_expansions import W16B_GLOSSARY_CLUSTER
from wave22_expansions import W22_GLOSSARY_CLUSTER
from wave23_expansions import (
    BEGINNER_SLUGS,
    CHECKLIST_SLUGS,
    GLOSSARY_SLUGS,
    HOWTO_SLUGS,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave23.cs"
...
def body(slug: str) -> str:
    ...
    if slug in BEGINNER_SLUGS:
        cluster = W23_BEGINNER_CLUSTER
        mega = COMPLIANCE_MEGA + SHARED_MEGA
    elif slug in HOWTO_SLUGS:
        cluster = W23_HOWTO_CLUSTER + CONVERT_CLUSTER + EDIT_CLUSTER
        mega = CONVERT_MEGA + COMPRESS_MEGA
    elif slug in CHECKLIST_SLUGS:
        cluster = W23_HOWTO_CLUSTER + COMPLIANCE_CLUSTER_LINKS + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + COMPLIANCE_MEGA
    else:
        cluster = W23_GLOSSARY_CLUSTER + W22_GLOSSARY_CLUSTER + W16_GLOSSARY_CLUSTER + W16B_GLOSSARY_CLUSTER
        mega = W16_MEGA
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 23 — Beginner basics + how-to spokes + checklist + glossary batch 5.</summary>",
        "    internal static class GuideBodiesWave23",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W23_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W23_SLUGS)} guides)")
