#!/usr/bin/env python3
"""Generate GuideBodiesWave22.cs — Industry tools + research + glossary batch 4."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave22.cs"
...
def body(slug: str) -> str:
    ...
    if slug in INDUSTRY_SLUGS:
        cluster = W22_INDUSTRY_CLUSTER
        mega = COMPARE_MEGA + COMPARE_SMB + COMPRESS_MEGA
    elif slug in RESEARCH_SLUGS:
        cluster = W22_RESEARCH_CLUSTER + COMPLIANCE_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + COMPLIANCE_MEGA
    else:
        cluster = W22_GLOSSARY_CLUSTER + W16_GLOSSARY_CLUSTER + W16B_GLOSSARY_CLUSTER
        mega = W16_MEGA
    ...
    return f"""
            ...
            {shot(d['title'])}
            {W22_DEEP[slug]}
            {mid_cta(d['tool'], d['label'], 'Open primary tool')}
            {mega}
            {COMPLIANCE_UNIVERSAL}
            {SHARED_MEGA}
            {SHARED_PILLAR}
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 22 — Industry PDF tools + research guides + glossary batch 4.</summary>",
        "    internal static class GuideBodiesWave22",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W22_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W22_SLUGS)} guides)")
