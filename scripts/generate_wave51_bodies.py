#!/usr/bin/env python3
"""Generate GuideBodiesWave51.cs — CREATE batch 15 (17 editorial guides)."""
...
from wave50_expansions import W50_MERGE_FOR_CLUSTER
from wave51_expansions import (
    DEVICE_CREATE_SLUGS,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave51.cs"
...
def body(slug: str) -> str:
    ...
    if slug in MERGE_FOR_CREATE_SLUGS:
        cluster = (
            W51_CREATE_CLUSTER
            + W51_MERGE_FOR_CLUSTER
        ...
        topup = W6_1500 + W51_COMPRESS_TOPUP
        closing = W51_CLOSING + W6_CLOSING
    elif slug in DEVICE_CREATE_SLUGS:
        cluster = W51_CREATE_CLUSTER + W51_DEVICE_CLUSTER + W14_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = PDF_WORD_MEGA + SHARED_MEGA + COMPRESS_MEGA
        topup = W14_1500_TOPUP + W51_COMPRESS_TOPUP
        closing = W51_CLOSING + W14_CLOSING
    else:
        cluster = W51_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W51_COMPRESS_TOPUP
        closing = W51_CLOSING
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 51 — CREATE batch 15 (17 editorial guides).</summary>",
        "    internal static class GuideBodiesWave51",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W51_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W51_SLUGS)} guides)")
