#!/usr/bin/env python3
"""Generate GuideBodiesWave39.cs — CREATE batch 3 (17 new editorial guides)."""
...
from wave39_expansions import (
    BUSINESS_CREATE_SLUGS,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave39.cs"
...
def body(slug: str) -> str:
    ...
    if slug in VISA_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + W38_CREATE_CLUSTER + W38_VISA_TOPUP + W21_REGIONAL_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + COMPLIANCE_MEGA
        topup = W39_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W39_CLOSING + W5_CLOSING
    elif slug in GOVT_CREATE_SLUGS or slug in COMPRESS_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + W30_COMPRESS_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + COMPLIANCE_MEGA
        topup = W39_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W39_CLOSING + W5_CLOSING
    elif slug in STUDENT_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + W39_STUDENT_TOPUP + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500 + W39_COMPRESS_TOPUP
        closing = W39_CLOSING + W6_CLOSING
    elif slug in JOB_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500 + W39_COMPRESS_TOPUP
        closing = W39_CLOSING + W6_CLOSING
        ...
        topup = W6_1500 + W39_COMPRESS_TOPUP
        closing = W39_CLOSING + W6_CLOSING
    elif slug in BUSINESS_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + SHARED_MEGA
        topup = W39_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W39_CLOSING + W5_CLOSING
    elif slug in OCR_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + W25_OCR_CLUSTER + EDIT_CLUSTER
        mega = COMPRESS_MEGA + SHARED_MEGA
        topup = W12_1500_TOPUP
        closing = W39_CLOSING + W6_CLOSING
    elif slug in COMPARE_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + W12_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPARE_MEGA + COMPARE_SMB + COMPARE_DEEP + SHARED_MEGA
        topup = W12_1500_TOPUP
        closing = W39_CLOSING + W6_CLOSING
    elif slug in CONVERT_CREATE_SLUGS or slug in DEVICE_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + EDIT_CLUSTER + CONVERT_MEGA
        mega = CONVERT_MEGA + SHARED_MEGA
        topup = W6_1500
        closing = W39_CLOSING + W6_CLOSING
    elif slug in FORM_CREATE_SLUGS:
        cluster = W39_CREATE_CLUSTER + EDIT_CLUSTER + W16_GLOSSARY_CLUSTER
        ...
        cluster = W39_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W39_COMPRESS_TOPUP
        closing = W39_CLOSING
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 39 — CREATE batch 3 (17 new editorial guides).</summary>",
        "    internal static class GuideBodiesWave39",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W39_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W39_SLUGS)} guides)")
