#!/usr/bin/env python3
"""Generate GuideBodiesWave28.cs — Regional contract batch 6 + tax document batch 1."""
...
from wave27_expansions import W27_CONTRACT_CLUSTER
from wave28_expansions import (
    CONTRACT_SLUGS,
    TAX_SLUGS,
    W28_CLOSING,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave28.cs"
...
def body(slug: str) -> str:
    ...
    if slug in CONTRACT_SLUGS:
        cluster = W28_CONTRACT_CLUSTER + W27_CONTRACT_CLUSTER + W21_REGIONAL_CLUSTER
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W28_CONTRACT_TOPUP
    else:
        cluster = W28_TAX_CLUSTER + W21_REGIONAL_CLUSTER + COMPRESS_CLUSTER_LINKS
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 28 — Regional contract batch 6 + tax document batch 1.</summary>",
        "    internal static class GuideBodiesWave28",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W28_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W28_SLUGS)} guides)")
