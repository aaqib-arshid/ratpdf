#!/usr/bin/env python3
"""Generate GuideBodiesWave12.cs — Standards & archiving cluster (17 pages, 1500+ words)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave12.cs"
...
def cluster_long(tool: str, focus: str) -> str:
    return f"""
            ...
            <p>Archival PDFs need policy before tools — standard selection beats retroactive conversion.</p>
            {shot(focus)}
            <h2>Validation</h2>
            <p>Certified PDF/A/UA/X requires validator output in audit file — not assumption from export dialog.</p>
            <div class="rp-faq-cta alert alert-secondary my-4"><strong>Inspect</strong> <a href="{tool}" class="alert-link fw-semibold">Go →</a></div>
            """
...
def body(slug: str) -> str:
    d = W12_DATA[slug]
    pillar_note = (
        '<p>Hub: <a href="/guides/pdf-standards-archiving-hub">standards &amp; archiving hub</a> · '
        '<a href="/guides/pdf-standards-guide">standards matrix</a>.</p>'
    ...
    return f"""
            ...
            {W6_CLOSING}
            {W5_CLOSING}
            {end_cta(d['tool'], d['label'], '<a href="/guides/pdf-standards-archiving-hub">Standards hub</a>')}
            """
...
def main():
    lines = [
        ...
        "{",
        "    /// <summary>Wave 12 — PDF standards & archiving (PDF/A, PDF/UA, metadata, retention).</summary>",
        "    internal static class GuideBodiesWave12",
        "    {",
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W12_SLUGS)} guides)")
