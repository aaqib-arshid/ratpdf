#!/usr/bin/env python3
"""Generate GuideBodiesWave32.cs — Glossary authority batch 9."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave32.cs"
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 32 — Glossary authority batch 9.</summary>",
        "    internal static class GuideBodiesWave32",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W32_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W32_SLUGS)} guides)")
