#!/usr/bin/env python3
"""Generate GuideBodiesWave46.cs — CREATE batch 10 (17 new editorial guides)."""
...
from wave44_expansions import W44_CREATE_CLUSTER
from wave45_expansions import W45_CREATE_CLUSTER
from wave46_expansions import (
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave46.cs"
...
def body(slug: str) -> str:
    ...
    if slug in ARCHIVAL_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + W46_ARCHIVAL_CLUSTER + W44_CREATE_CLUSTER + W16_GLOSSARY_CLUSTER + EDIT_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP + W46_COMPRESS_TOPUP
        closing = W46_CLOSING + W6_CLOSING
    elif slug in WORKFLOW_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + EDIT_CLUSTER + W45_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W46_CLOSING + W6_CLOSING
    elif slug in TROUBLESHOOT_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W46_CLOSING + W6_CLOSING
    elif slug in FORM_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + W46_FORM_CLUSTER + EDIT_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W6_1500
        closing = W46_CLOSING + W6_CLOSING
    elif slug in PRINT_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + W46_PRINT_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W46_CLOSING + W6_CLOSING
    elif slug in SECURITY_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + W11_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = W11_MEGA + COMPLIANCE_MEGA
        topup = W11_1500_TOPUP
        closing = W46_CLOSING + W11_CLOSING
    elif slug in COMPRESS_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + W30_COMPRESS_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + SHARED_MEGA
        topup = W46_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W46_CLOSING + W5_CLOSING
    elif slug in ACCESSIBILITY_CREATE_SLUGS:
        cluster = W46_CREATE_CLUSTER + W25_OCR_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        ...
        cluster = W46_CREATE_CLUSTER + W16_GLOSSARY_CLUSTER + SHARED_PILLAR
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W5_1500_TOPUP
        closing = W46_CLOSING + W5_CLOSING
    else:
        cluster = W46_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W46_COMPRESS_TOPUP
        closing = W46_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W46_SLUGS)} guides)")
