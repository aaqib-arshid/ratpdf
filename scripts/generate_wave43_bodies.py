#!/usr/bin/env python3
"""Generate GuideBodiesWave43.cs — CREATE batch 7 (17 new editorial guides)."""
...
from wave42_expansions import W42_BANK_CLUSTER, W42_CREATE_CLUSTER
from wave43_expansions import (
    ACCESSIBILITY_CREATE_SLUGS,
    BANK_CREATE_SLUGS,
    FORM_CREATE_SLUGS,
    METADATA_CREATE_SLUGS,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave43.cs"
...
def body(slug: str) -> str:
    ...
    if slug in BANK_CREATE_SLUGS:
        ...
        topup = W43_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W43_CLOSING + W5_CLOSING
    elif slug in FORM_CREATE_SLUGS:
        cluster = W43_CREATE_CLUSTER + EDIT_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W6_1500
        closing = W43_CLOSING + W6_CLOSING
    elif slug in METADATA_CREATE_SLUGS:
        cluster = W43_CREATE_CLUSTER + EDIT_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W43_CLOSING + W6_CLOSING
    elif slug in ACCESSIBILITY_CREATE_SLUGS:
        cluster = W43_CREATE_CLUSTER + W25_OCR_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W43_CLOSING + W6_CLOSING
    elif slug in TROUBLESHOOT_CREATE_SLUGS:
        cluster = W43_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W43_CLOSING + W6_CLOSING
    else:
        cluster = W43_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W43_COMPRESS_TOPUP
        closing = W43_CLOSING
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 43 — CREATE batch 7 (17 new editorial guides).</summary>",
        "    internal static class GuideBodiesWave43",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W43_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W43_SLUGS)} guides)")
