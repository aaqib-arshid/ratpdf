#!/usr/bin/env python3
"""Generate GuideBodiesWave55.cs — CREATE batch 19 (17 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave55.cs"
...
def body(slug: str) -> str:
    ...
    if slug in OCR_FOR_CREATE_SLUGS:
        ...
        topup = W6_1500 + W55_COMPRESS_TOPUP
        closing = W55_CLOSING + W6_CLOSING
    elif slug in WORKFLOW_CREATE_SLUGS:
        cluster = W55_CREATE_CLUSTER + W25_OCR_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W6_1500
        closing = W55_CLOSING + W6_CLOSING
    else:
        cluster = W55_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W55_COMPRESS_TOPUP
        closing = W55_CLOSING
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 55 — CREATE batch 19 (17 editorial guides).</summary>",
        "    internal static class GuideBodiesWave55",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W55_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W55_SLUGS)} guides)")
