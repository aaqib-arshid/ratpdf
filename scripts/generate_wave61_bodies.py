#!/usr/bin/env python3
"""Generate GuideBodiesWave61.cs — CREATE batch 25 (17 editorial guides)."""
...
from wave61_expansions import (
    CONVERTER_CREATE_SLUGS,
    W61_CLOSING,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave61.cs"
...
def body(slug: str) -> str:
    ...
    topup = W6_1500 + W61_TOPUP
    closing = W61_CLOSING + W6_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W61_SLUGS)} guides)")
