#!/usr/bin/env python3
"""Generate GuideBodiesWave63.cs — CREATE batch 27 (final 2 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave63.cs"
...
def body(slug: str) -> str:
    ...
    cluster = W63_CREATE_CLUSTER + W25_OCR_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
    mega = COMPARE_MEGA + COMPARE_SMB + COMPARE_DEEP + SHARED_MEGA + COMPLIANCE_MEGA
    topup = W6_1500 + W63_TOPUP
    closing = W63_CLOSING + W6_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W63_SLUGS)} guides)")
