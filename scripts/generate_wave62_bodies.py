#!/usr/bin/env python3
"""Generate GuideBodiesWave62.cs — CREATE batch 26 (17 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave62.cs"
...
def body(slug: str) -> str:
    ...
    topup = W6_1500 + W62_TOPUP
    closing = W62_CLOSING + W6_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W62_SLUGS)} guides)")
