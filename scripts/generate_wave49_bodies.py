#!/usr/bin/env python3
"""Generate GuideBodiesWave49.cs — CREATE batch 13 (17 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave49.cs"
...
def body(slug: str) -> str:
    ...
    if slug in PERSONA_CREATE_SLUGS:
        cluster = W49_CREATE_CLUSTER + W49_PERSONA_CLUSTER + W48_PERSONA_CLUSTER + COMPRESS_CLUSTER_LINKS + EDIT_CLUSTER
        mega = COMPARE_MEGA + COMPARE_SMB + COMPARE_DEEP + SHARED_MEGA
        topup = W6_1500 + W49_COMPRESS_TOPUP
        closing = W49_CLOSING + W6_CLOSING
    elif slug in ARCHIVAL_CREATE_SLUGS:
        cluster = W49_CREATE_CLUSTER + W46_ARCHIVAL_CLUSTER + W25_OCR_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W49_CLOSING + W6_CLOSING
    elif slug in WORKFLOW_CREATE_SLUGS:
        cluster = W49_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        ...
        cluster = W49_CREATE_CLUSTER + W30_COMPRESS_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + SHARED_MEGA
        topup = W49_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W49_CLOSING + W5_CLOSING
    elif slug in FORM_CREATE_SLUGS or slug in ACCESSIBILITY_CREATE_SLUGS:
        cluster = W49_CREATE_CLUSTER + W25_OCR_CLUSTER + W16_GLOSSARY_CLUSTER + EDIT_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W49_CLOSING + W6_CLOSING
    else:
        cluster = W49_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W49_COMPRESS_TOPUP
        closing = W49_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W49_SLUGS)} guides)")
