#!/usr/bin/env python3
"""Generate GuideBodiesWave36.cs — Glossary final batch; completes UPGRADE queue."""
...
from wave34_expansions import W34_GLOSSARY_CLUSTER
from wave35_expansions import W35_GLOSSARY_CLUSTER
from wave36_expansions import (
    W36_CLOSING,
    W36_DATA,
    W36_DEEP,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave36.cs"
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W36_SLUGS)} guides)")
