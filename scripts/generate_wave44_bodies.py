#!/usr/bin/env python3
"""Generate GuideBodiesWave44.cs — CREATE batch 8 (17 new editorial guides)."""
...
from wave42_expansions import W42_CREATE_CLUSTER
from wave43_expansions import W43_CREATE_CLUSTER
from wave44_expansions import (
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave44.cs"
...
def body(slug: str) -> str:
    ...
    if slug in CONTRACT_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + W44_CONTRACT_CLUSTER + W21_REGIONAL_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA + COMPLIANCE_MEGA
        topup = W6_1500 + W44_COMPRESS_TOPUP
        closing = W44_CLOSING + W6_CLOSING
    elif slug in GOVT_CREATE_SLUGS or slug in COMPRESS_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + W30_COMPRESS_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + COMPLIANCE_MEGA
        topup = W44_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W44_CLOSING + W5_CLOSING
    elif slug in COMPARE_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + W12_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPARE_MEGA + COMPARE_SMB + COMPARE_DEEP + SHARED_MEGA
        topup = W12_1500_TOPUP
        closing = W44_CLOSING + W6_CLOSING
    elif slug in SECURITY_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + W11_CLUSTER
        mega = W11_MEGA + COMPLIANCE_MEGA
        topup = W11_1500_TOPUP
        closing = W44_CLOSING + W11_CLOSING
    elif slug in ARCHIVAL_CREATE_SLUGS or slug in PRINT_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + W16_GLOSSARY_CLUSTER + EDIT_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W44_CLOSING + W6_CLOSING
    elif slug in ACCESSIBILITY_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + W25_OCR_CLUSTER + W16_GLOSSARY_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W44_CLOSING + W6_CLOSING
    elif slug in TROUBLESHOOT_CREATE_SLUGS:
        cluster = W44_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W44_CLOSING + W6_CLOSING
    else:
        cluster = W44_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W44_COMPRESS_TOPUP
        closing = W44_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W44_SLUGS)} guides)")
