#!/usr/bin/env python3
"""Generate GuideBodiesWave42.cs — CREATE batch 6 (17 new editorial guides)."""
...
from wave41_expansions import W41_CREATE_CLUSTER, W41_ASIA_VISA_CLUSTER
from wave42_expansions import (
    ACCOUNTANT_CREATE_SLUGS,
    ARCHIVAL_CREATE_SLUGS,
    BANK_CREATE_SLUGS,
    DEVICE_CREATE_SLUGS,
    GOVT_CREATE_SLUGS,
    RETENTION_CREATE_SLUGS,
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave42.cs"
...
def body(slug: str) -> str:
    ...
    if slug in BANK_CREATE_SLUGS:
        ...
        topup = W42_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W42_CLOSING + W5_CLOSING
    elif slug in GOVT_CREATE_SLUGS or slug in STUDENT_CREATE_SLUGS:
        cluster = W42_CREATE_CLUSTER + W30_COMPRESS_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA + COMPRESS_LONG_TAIL + COMPLIANCE_MEGA
        topup = W42_COMPRESS_TOPUP + W12_1500_TOPUP
        closing = W42_CLOSING + W5_CLOSING
    elif slug in SECURITY_CREATE_SLUGS or slug in ACCOUNTANT_CREATE_SLUGS:
        cluster = W42_CREATE_CLUSTER + W11_CLUSTER
        mega = W11_MEGA + COMPLIANCE_MEGA
        topup = W11_1500_TOPUP
        closing = W42_CLOSING + W11_CLOSING
    elif slug in ARCHIVAL_CREATE_SLUGS or slug in RETENTION_CREATE_SLUGS:
        cluster = W42_CREATE_CLUSTER + W16_GLOSSARY_CLUSTER + EDIT_CLUSTER
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W12_1500_TOPUP
        closing = W42_CLOSING + W6_CLOSING
    elif slug in DEVICE_CREATE_SLUGS:
        cluster = W42_CREATE_CLUSTER + EDIT_CLUSTER + CONVERT_MEGA
        mega = CONVERT_MEGA + SHARED_MEGA
        topup = W6_1500
        closing = W42_CLOSING + W6_CLOSING
    elif slug in TROUBLESHOOT_CREATE_SLUGS:
        cluster = W42_CREATE_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPRESS_MEGA
        topup = W6_1500
        closing = W42_CLOSING + W6_CLOSING
    else:
        cluster = W42_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W42_COMPRESS_TOPUP
        closing = W42_CLOSING
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 42 — CREATE batch 6 (17 new editorial guides).</summary>",
        "    internal static class GuideBodiesWave42",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W42_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W42_SLUGS)} guides)")
