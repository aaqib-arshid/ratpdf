#!/usr/bin/env python3
"""Generate GuideBodiesWave59.cs — CREATE batch 23 (17 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave59.cs"
...
def body(slug: str) -> str:
    ...
    mega = COMPRESS_MEGA + COMPARE_MEGA + COMPARE_SMB + COMPARE_DEEP + SHARED_MEGA + COMPLIANCE_MEGA
    topup = W6_1500 + W59_TOPUP
    closing = W59_CLOSING + W6_CLOSING
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W59_SLUGS)} guides)")
