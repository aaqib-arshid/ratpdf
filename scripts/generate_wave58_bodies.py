#!/usr/bin/env python3
"""Generate GuideBodiesWave58.cs — CREATE batch 22 (17 editorial guides)."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave58.cs"
...
def body(slug: str) -> str:
    ...
    if slug in SECURE_CREATE_SLUGS:
        cluster = (
            W58_CREATE_CLUSTER
            + W58_SECURE_CLUSTER
        ...
        topup = W6_1500 + W58_COMPRESS_TOPUP
        closing = W58_CLOSING + W6_CLOSING
    elif slug in WORKFLOW_CREATE_SLUGS:
        cluster = W58_CREATE_CLUSTER + W16_GLOSSARY_CLUSTER + EDIT_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = SHARED_MEGA + COMPLIANCE_MEGA
        topup = W6_1500 + W58_COMPRESS_TOPUP
        closing = W58_CLOSING + W6_CLOSING
    else:
        cluster = W58_CREATE_CLUSTER + COMPRESS_CLUSTER_LINKS
        mega = COMPRESS_MEGA
        topup = W58_COMPRESS_TOPUP
        closing = W58_CLOSING
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 58 — CREATE batch 22 (17 editorial guides).</summary>",
        "    internal static class GuideBodiesWave58",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W58_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W58_SLUGS)} guides)")
