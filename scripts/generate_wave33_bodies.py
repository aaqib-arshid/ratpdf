#!/usr/bin/env python3
"""Generate GuideBodiesWave33.cs — Glossary authority batch 10."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave33.cs"
...
def main() -> None:
    lines = [
        ...
        "{",
        "    /// <summary>Wave 33 — Glossary authority batch 10.</summary>",
        "    internal static class GuideBodiesWave33",
        "    {",
        "        public static string? Get(string slug) => slug switch",
        "        {",
    ]
    for slug in W33_SLUGS:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W33_SLUGS)} guides)")
