#!/usr/bin/env python3
"""Generate GuideBodiesWave35.cs — Glossary authority batch 12."""
...
OUT = Path(__file__).resolve().parents[1] / "Content" / "GuideBodiesWave35.cs"
...
def main() -> None:
    ...
    lines += ["    }", "}"]
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {OUT} ({len(W35_SLUGS)} guides)")
